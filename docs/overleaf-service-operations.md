...
推荐的 self-hosted Overleaf 路径仍是官方 Overleaf Toolkit / Docker Compose。裸机源码部署理论上可行，但需要自己维护 Node、Redis、Mongo、编译服务、历史服务、反向代理和升级兼容性，运维成本明显更高。
...
Agent / script
  ...
  -> OVERLEAF_SITE_URL
  -> self-hosted Overleaf web service
...
- CLI JSON 默认不输出内部 compile URL、owner/updater metadata。
- compile output 下载会拒绝 cross-origin URL，避免把认证 header 或 cookie 发给非 Overleaf host。
- mutation 类能力尚未实现；未来文件删除、sync push、admin 操作必须默认 dry-run 或要求显式 `--apply`。
